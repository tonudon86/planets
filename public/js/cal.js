...
!(function() {
  ...
  var settings = { 
    ...
    formFields: {"location": true, "general": true, "stars": true, "dsos": true, "constellations": true, "lines": true, "other": true, download: true},
  ...
  */
  ...
  function setVisibility(cfg, which) {
     ...
     if (which && has(cfg.formFields, which)) {
       d3.select(parentElement + " ~ #celestial-form").select("#" + which).style( {"display": ""} );
       return;
     }
     ...
     if (cfg.form === false && cfg.location === true) {
       ...
       for (fld in cfg.formFields) {
        if (!has(cfg.formFields, fld)) continue;
         ...
         d3.select(parentElement + " ~ #celestial-form").select("#" + fld).style( {"display": ""} );     
       }
       return;
     }
     // hide if not desired
  ...
  function geo(cfg) {
    ...
    var col = frm.append("div").attr("class", "col").attr("id", "location").style("display", "block");
    ...
    if (has(config, "formFields") && (config.location === true || config.formFields.location === true)) {
      d3.select(parentElement + " ~ #celestial-form").select("#location").style( {"display": "block"} );
    }
    //only if appropriate
